...
if __name__ == "__main__":
    ...
    if  basename_folder not in os.listdir(ruta_salida):
        os.mkdir(join(ruta_salida,basename_folder))

    for c in classes:
        if c not in os.listdir(join(ruta_salida,basename_folder)):
            os.mkdir(join(ruta_salida,basename_folder,c))

    for i in os.listdir(opt.image_folder):
        frame = cv2.imread(join(opt.image_folder,i))
        frame = cv2.resize(frame, (500, 700), interpolation=cv2.INTER_CUBIC)
            ...
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    ...
                    try:                    
                        ...
                        f = join(ruta_salida,basename_folder,classes[int(cls_pred)],i)
                        if i not in os.listdir(join(ruta_salida,basename_folder,classes[int(cls_pred)])):
