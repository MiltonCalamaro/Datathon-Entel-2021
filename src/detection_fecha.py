...
if __name__ == "__main__":
    ...
    ruta_salida = opt.image_folder
    if 'digitos' not in os.listdir(ruta_salida):
        os.mkdir(join(ruta_salida,'digitos'))

    for c in classes:
        if c not in os.listdir(join(ruta_salida,'digitos')):
            os.mkdir(join(ruta_salida,'digitos',c))

    for i in os.listdir(opt.image_folder):
        import time 
        time.sleep(1)
        frame = cv2.imread(join(opt.image_folder,i))
        frame= cv2.copyMakeBorder(frame,100,100,100,100,cv2.BORDER_CONSTANT,value=0)
        #frame = cv2.resize(frame, (800, 300), interpolation=cv2.INTER_CUBIC)
        frame_copy = frame.copy()
        #LA imagen viene en Blue, Green, Red y la convertimos a RGB que es la entrada que requiere el modelo
        try:
            RGBimg=Convertir_RGB(frame)
        except:
            print(f'error con archivo de {i}')
            ...
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    ...
                    try:                    
                        ...
                        f = join(ruta_salida,'digitos',classes[int(cls_pred)],i )
                        if i not in os.listdir(join(ruta_salida,'digitos',classes[int(cls_pred)])):
